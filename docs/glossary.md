...
- **Enrichment:** The bronze → silver step that adds external context (abuse history, scanner classification, exposed services, malware reputation, phishing presence, GeoIP/ASN attribution) to each unique attacker IP via six third-party providers. See [`integrations.md`](integrations.md).
- **Pseudonymization (OPSEC redaction):** The deterministic replacement of operation-related infrastructure IPs (external WAN, internal subnets, sensor/collector addresses) with stable labels (e.g. `honeypot-wan`, `honeypot-sensor-01`) before any data leaves the silver layer. Driven by the `redact.pseudonym_map` block in `reporting.yml`.
...
- **Vault:** The Ansible-Vault-encrypted `vault.yml` file inside each operation's `group_vars/all/`. Holds enrichment API keys, the Discord webhook, and (optionally) the sudo password. Decrypted at deploy time and rendered verbatim into `/etc/lantana/collector/secrets.json` on the collector. See [`runbook.md`](runbook.md#5-create-the-vault).
- **Narrative:** The deception story applied to a sensor: company identity, fake host profile (OS, kernel, hostname), and service banners. Lives in `narrative.yml` per operation. Distinct from the persona's *real* infrastructure, which never appears in attacker-facing artifacts.
